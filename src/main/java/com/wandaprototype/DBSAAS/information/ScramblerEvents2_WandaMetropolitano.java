...
public class ScramblerEvents2_WandaMetropolitano {
	...
	public void ExtractDataFromURL() {
		...
		Elements links = null, links2 = null, links3 = null;
		...
		try {
			...
			// links = doc.select("span.picture, article:matches((noticia)noticia) >
			// time[datetime], article.noticia > a[href]");
							...
							for (Element e2 : links2) {
								if (!e2.text().isEmpty()) {
									...
									if (matcher.find()) {
										...
										try {
											doc3 = Jsoup.connect("https://www.atleticodemadrid.com" + e.attributes().get("href")).get();
											// links2 = doc2.select("span.preheading");
											links3 = doc3.select("article.noticia-interior > p");
											
											for (Element e3 : links3) {
												if (!e3.text().isEmpty()) {
													matcher = Pattern.compile(regex).matcher(e3.text().toString());
													if (matcher.find()) {
														if (!matcher.group().contains("vs")) {
	...
	public static void main(String[] args) {
		new ScramblerEvents2_WandaMetropolitano().ExtractDataFromURL();
		new ScramblerEvents2_WandaMetropolitano().ClasifDatos();
		new ScramblerEvents2_WandaMetropolitano().LecturaDatosParticionados();
	}
}
