...
public class ScramblerMatchs_Atletico_Madrid {
	...
	public void ExtractDataFromURL() {
		...
		for (Element e : links) {
			...
			if (e.text().contentEquals("Resumen") || e.text().contentEquals("AVÃ­SAME")
					|| e.text().contentEquals("ENTRADAS") || e.text().contentEquals("Previa")
					|| e.text().contentEquals("Â¡En directo!") || e.text().isEmpty()) {
			} else {
				...
				if (lists.get(c).toString().contains(DameDatosFecha.meses.get(ds).toString())) {
					...
					for (int i = 0; i < stringarray.length; i++) {
						if (stringarray[i].equals(" ") || stringarray[i].equals("de") || stringarray[i].equals("-")
							...
							/*
							 * for (int i2=0; i2<meses.size(); i2++) {
								if (stringarray[i].toString().equalsIgnoreCase(meses.get(i2))) {
							 */
							...
							*/
							...
							/*
							 * for (int i2=0; i2<dias.size(); i2++) {
								if (stringarray[i].toString().equalsIgnoreCase(dias.get(i2))) {
							 */
